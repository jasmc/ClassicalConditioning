"""Assess the frozen legacy-metric preview using the saved hashed intake."""
...
def main() -> None:
    ...
    if (len(ids) != snapshot["selected_count"] or len(set(ids)) != len(ids)
            or snapshot["metric_id"] != "legacy_distal_angular_speed"):
        raise ValueError("Legacy preview snapshot identity is invalid")
    ...
    result = assess_discarding(
        RAW, PROJECT,
        analysis_id="all3sTrace-window13-legacy-53fish",
