...
def reference_tail_length_px(
    ...
    y: np.ndarray,
    *,
    minimum_valid_tail_fraction: float,
) -> float:
    """Calculate one robust anatomical scale from eligible recording frames."""
    segment_lengths = np.hypot(np.diff(x, axis=1), np.diff(y, axis=1))
    valid = np.isfinite(segment_lengths) & (segment_lengths > 0)
    valid_fraction = np.mean(valid, axis=1)
    arc_length = np.sum(np.where(valid, segment_lengths, 0.0), axis=1)
    eligible = (
        (valid_fraction >= minimum_valid_tail_fraction)
    ...
    if not np.isfinite(result) or result <= 0:
        raise ValueError("Reference tail length must be positive and finite.")
    return result


def reference_tail_length_from_parquet(
    ...
    *,
    ...
    """Calculate the recording-wide median tail length without loading XY in full."""
    columns = [
        f"{axis}{index}"
        for axis in ("x", "y")
        for index in range(point_count)
    ]
    eligible_lengths: list[np.ndarray] = []
    for batch in parquet_file.iter_batches(batch_size=batch_size, columns=columns):
        frame = batch.to_pandas()
        x = frame[[f"x{index}" for index in range(point_count)]].to_numpy(float)
        y = frame[[f"y{index}" for index in range(point_count)]].to_numpy(float)
        segment_lengths = np.hypot(np.diff(x, axis=1), np.diff(y, axis=1))
        valid = np.isfinite(segment_lengths) & (segment_lengths > 0)
        arc_length = np.sum(np.where(valid, segment_lengths, 0.0), axis=1)
        eligible = (
            (np.mean(valid, axis=1) >= minimum_valid_tail_fraction)
            & np.isfinite(arc_length)
            & (arc_length > 0)
        )
        if np.any(eligible):
            eligible_lengths.append(arc_length[eligible])
    if not eligible_lengths:
        raise ValueError("No frames have enough valid tail length for normalization.")
    result = float(np.median(np.concatenate(eligible_lengths)))
    if not np.isfinite(result) or result <= 0:
        raise ValueError("Reference tail length must be positive and finite.")
    return result
...
def calculate_candidate_metrics(
    ...
    *,
    ...
    reference_tail_length: float | None = None,
    ...
    if reference_tail_length is None and previous is not None:
        reference_tail_length = float(previous["reference_tail_length_px"])
    if reference_tail_length is None:
        reference_tail_length = reference_tail_length_px(
            body_x,
            body_y,
            minimum_valid_tail_fraction=config.minimum_valid_tail_fraction,
        )
    if not np.isfinite(reference_tail_length) or reference_tail_length <= 0:
        raise ValueError("Reference tail length must be positive and finite.")
    xy_mean_px_per_ms, valid_tail_fraction = _weighted_mean(
        point_speed,
        point_weights,
        config.minimum_valid_tail_fraction,
    )
    xy_mean = xy_mean_px_per_ms / reference_tail_length
    ...
    angular_l1, angular_valid_fraction = _weighted_mean(
        angular_speed,
        segment_weights,
        config.minimum_valid_tail_fraction,
    )
...
def build_direct_intake_candidate_metrics(
    project_dir: Path,
    recording_id: str,
    *,
