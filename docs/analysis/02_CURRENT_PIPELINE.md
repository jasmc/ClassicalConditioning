...
cli.py -> run_config.py -> pipeline.py
  ...
  -> bounded two-process renderer
       -> figures/temporal_profiles.py (5 CS families, 4 US families per ready fish)
...
2. `intake.py` compares each selected complete triplet with the source
   ...
   changed source. After corrected per-fish outcomes have run,
   `analysis/discarding.py` assesses all inventory records, including
   processing failures, before cohort comparison. Its technical and
   exploratory outputs do not make a reviewed inclusion decision.
