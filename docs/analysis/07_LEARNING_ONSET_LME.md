...
The experiment definitions specify 9/13/20 s response windows for Delay,
3sTrace, and 10sTrace. [`trial_outcome_config_for_experiment`](../../src/classical_conditioning/analysis/trial_outcomes.py)
passes the selected window into per-trial aggregation, and the resolved run
configuration records it. Older Trace artifacts may have used the former 9 s
default; current loaders reject those mismatched configs. Rebuild the
per-recording and cohort outcomes before refitting a Trace LME. The
[parameter index](03_ANALYSIS_PARAMETER_INDEX.md#exact-fixed-recipe-fields)
