...
The implementation is in `src/classical_conditioning/analysis/figure4.py`, `src/classical_conditioning/figures/figure4.py`, the `figure4-analyze` and `figure4-render` CLI routes, and the `figure4` paper-panel registry route. The input contract is in [the Figure 4 guide](./FIGURE4_LEARNER_PROFILES.md). The separate timing follow-on is [Plan 11](../../../Plans/11_S7_RESPONSE_TIMING.md).
