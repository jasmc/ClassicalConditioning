...
Expected-US guides are derived from authenticated paired-training `Reinforcer` events and compared with `ExperimentSpec`. The command stops if they disagree, including a 3sTrace 9-versus-13-second discrepancy. Catch and non-US rows show the verified paired-training expectation, not an observed US. Same-data learner curves carry no independent group-inference claim or p-values. The future S7 timing analysis is specified in [the timing plan](../../../Plans/11_S7_RESPONSE_TIMING.md).
