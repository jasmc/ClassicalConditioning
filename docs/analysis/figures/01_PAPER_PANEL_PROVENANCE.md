...
The supported integrated command and example are in [02 specification § Rendering interface](02_PAPER_FIGURE_SPECIFICATION.md#rendering-interface). Its default review set renders Figure 1 C/D and paired Delay/control heatmaps, plus Delay Figure 2 A/D/G and separate heatmap coverage. The optional inference overlay is exploratory. Figure 4 uses `figure4-analyze` then `figure4-render` with three reviewed cohorts and a frozen learner manifest.
...
| **1B** protocol | Draw four contingencies and trial schedule from `ExperimentSpec` and generated `Metadata/trial_map_<experiment>.json`; distinguish 50 and 100 ms US pulses in approved protocol art. | Complete reviewed artwork absent. The configured 3sTrace paired US latency is 9 s while the paper scaffold expects 13 s; raw protocol identity must be reconciled. |
| **1C** tail angle | For selected fish and global CS trials, [`example_traces.py`](../../../src/classical_conditioning/figures/example_traces.py) reads corrected frames, centers distal cumulative angle on the trial pre-CS median, and plots measured frames with event guides. Integrated adapter: [`render_legacy_ssd_example_traces.py`](../../../scripts/render_legacy_ssd_example_traces.py). | Paired C/D review exists; representative fish/trials and final composition remain open. |
| **1D** vigor trace | Same fish/trials and time axis as 1C; plot one selected corrected frame metric. The optional orange step overlay is the exact 1E signed heatmap bins on a separately labeled axis; invalid gaps remain open. Renderer above; focused review: [`render_figure1_raw_vigor_y_focus.py`](../../../scripts/render_figure1_raw_vigor_y_focus.py). | Review exists; metric and examples need selection. Frame vigor is not silently rescaled to the heatmap color range. |
| **1E** Delay fish | [`signed_bout_heatmap.py`](../../../src/classical_conditioning/figures/signed_bout_heatmap.py) computes moving positive bout-frame log vigor relative to each trial's [−20, 0) s median, then 0.5 s signed bins. [`render_legacy_ssd_example_heatmaps.py`](../../../scripts/render_legacy_ssd_example_heatmaps.py) draws the provisional Delay/control pair with `managua_r`, fixed −0.25…+0.25, NaNs distinct. | Paired review exists for provisional fish `20221115_07`/`20221115_09`; single final panel, metric and example approval are open. |
| **1F** 3sTrace fish | Same signed single-fish method as 1E, applied to a prespecified 3sTrace fish and authenticated corrected artifacts. | No final fish selection or integrated paper-panel renderer. |
| **1G** unpaired control fish | Same signed method as 1E for a prespecified control fish; retain coverage mask and trial order. | Control half of the paired 1E review is a candidate, but no separately composed final G. |
