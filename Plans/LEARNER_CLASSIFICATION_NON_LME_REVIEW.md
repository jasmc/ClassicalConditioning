# Learner Identification Without a Linear Mixed-Effects Classifier
...
**Relationship:** Supplements [the active learner plan](./Analysis/2_LEARNER_CLASSIFICATION.md), [the analysis design](./Analysis/1_ANALYSIS_AND_STATISTICS.md), and [the decision register](./DECISIONS.md). It does not replace their cohort, outcome, validation, or approval requirements.
...
- Gate L requires a learner-focused paper result but leaves the continuous, longitudinal, probabilistic, and categorical representation open. A hard label is optional [active plan](./Analysis/2_LEARNER_CLASSIFICATION.md).
- The primary fish cohort must be fixed using label-independent technical criteria. The unified selection assessment supplies learner eligibility and reasons; the learner method must not run its own fish or row discard path [decision register](./DECISIONS.md).
