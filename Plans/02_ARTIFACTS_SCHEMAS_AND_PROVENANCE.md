...
Still required for the **priority lane** (current-path exit):

- migration of remaining route-specific metadata onto the shared hashing and
  transactional-publish contract where still incomplete.

Deferred per [DECISIONS.md](./DECISIONS.md) (do **not** block Steps 06–10
fixture work):
...
### Current-path exit (priority lane)

SHA-256 byte hashing and atomic transactional publication are accepted for
intake, legacy, candidate, movement, profile, figure, and trace-review routes
now used on fixtures. Explicit upstream marker verification remains required.

### Full Step 02 exit (deferred items)
...
canonical artifact has a schema, logical hash, provenance, and explicit
upstream IDs. This full exit is not required to continue fixture work on
Steps 06–10.
