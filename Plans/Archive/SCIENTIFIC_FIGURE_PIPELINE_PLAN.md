...
> **Implementation authority (historical):** Active sequencing remains governed
> by `MASTER_ANALYSIS_MIGRATION_PLAN.md` and `IMPLEMENTATION_STEP_INDEX.md`.
...
- Preserve existing interactive HTML as-is (frozen; no further investment).
...
## Workstream E: Interactive adjustment — FROZEN

> **Gate F:** Interactive local HTML figures are frozen as-is. Do not invest in
> new interactive layout widgets, Matplotlib-widget polish, or Inkscape-driven
> interactive workflows. Existing HTML outputs remain available for review.
> Maintained modes are static PNG and publication SVG/PDF only.
>
> Historical workstream text below is retained for archive context only and is
