...
**Change class:** Behavior-preserving legacy branch plus approved corrected outcomes  
**Prototype scope:** Synthetic and bounded single-fish candidate artifacts only
...
Implemented in `788602f`:

- one row per recording, alignment, trial, 0.5 s bin, and candidate metric;
- total activity including valid zeros;
- movement probability among detector-valid samples;
- measured-time fraction moving;
- conditional intensity among moving valid samples;
- bout count, bout rate, and mean duration;
- acquisition, metric, and detector coverage;
- explicit detector and metric identities;
- 94 CS and 79 US raw-protocol events.

Artifact:

```text
Processed data/20221115_04/candidate_temporal_outcomes-v2.parquet
```

This is a single-fish candidate artifact. It does not satisfy the cohort,
