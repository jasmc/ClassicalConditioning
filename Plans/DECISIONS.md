...
| C0 (cohort inclusion) | **Primary paper policy remains open; boundary decided.** One unified selection-assessment step calculates all technical, legacy Stage-1, Stage-5/LME, and learner-feature dispositions before cohort review/model/classifier execution. Primary membership may use only prespecified label-independent technical criteria. Response-window movement, CR strength, and learner status cannot determine it. The historical Stage-1 behavior screen is preserved non-destructively as `legacy-discard-interim-v1`, a legacy-equivalent sensitivity policy only: all required US checks and every selected baseline/CR block must pass (AND logic), and its effect is reported after learner status is available. The exact Stage-5 any-block OR bug is recorded for reproduction alongside an all-required-block sensitivity status; neither defines the primary cohort. Downstream LME and learner code may not discard independently. |
