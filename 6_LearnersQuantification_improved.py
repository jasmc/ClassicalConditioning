...
Outputs:
- `Fish_Learner_Classification_Improved_<CSUS>.csv` in the pooled data folder
"""
...
IGNORE_WARNINGS: bool = False  # set True to suppress; new script uses unconditional ignore
...
# ==============================================================================
# Toggle analysis stages to mirror Pipeline_Analysis.py / 6_LearnersQuantification_new.py behavior.
RUN_PIPELINE: bool = True
RUN_DIAGNOSTICS: bool = False
RUN_PLOT_DIAGNOSTICS: bool = False
RUN_PLOT_TRAJECTORIES: bool = False
RUN_PLOT_FEATURE_SPACE: bool = False
RUN_PLOT_BLUP_CATERPILLAR: bool = False
RUN_PLOT_INDIVIDUALS: bool = True
RUN_PLOT_INDIVIDUALS_AND_GRID: bool = True  # alias for individual + grid plots
RUN_PLOT_BLUP_OVERLAY: bool = True
RUN_PLOT_HEATMAP_GRID: bool = True
RUN_EXPORT_DETAILED_SUMMARY: bool = False
...
# ==============================================================================
# GENERAL RUNTIME SETTINGS (data loading / column conventions)
# ==============================================================================
...
# ==============================================================================
# FIGURE EXPORT DEFAULTS + OUTPUT NAMING
# ==============================================================================
...
FNAME_CLASSIFICATION_RESULTS_TEMPLATE: str = "Fish_Learner_Classification_Improved_{csus}.csv"
...
# Colors aligned with other analysis scripts for continuity
COLOR_PROB_LEARNER: str = "red"
COLOR_POINT_LEARNER: str = "gold"
...
# In the improved pipeline, p_dir = Phi(z_dir) with 0.5 meaning "at control".
# When scoring uses per-fish SE, p_dir is interpretable as a one-sided probability-like quantity.
# When scoring ignores per-fish SE, p_dir is still a monotone directional score (not a calibrated probability).
# For display, treat p_dir > 0.5 (equivalently z_dir > 0) as "passes direction".
P_DIR_DISPLAY_THRESHOLD: float = 0.5
...
def save_heatmap_grid(
    ...
    heatmap_files = list(Path(path_scaled_vigor_fig_cs).glob("*.svg")) + list(Path(path_scaled_vigor_fig_cs).glob("*.png"))
    ...
    title_suffix = f" - {str(condition).capitalize()}" if condition else ""
    fig.suptitle(f"Scaled Vigor Heatmaps (aligned to CS){title_suffix}", fontsize=5 + 16, y=0.99)
    ...
    cond_suffix = f"_{condition}" if condition else ""
    grid_path = Path(output_dir) / f"Heatmap_Grid_CS{cond_suffix}.png"
    figure_saving.save_figure(fig, grid_path, frmt="png", dpi=int(FIG_DPI_SUMMARY_GRID), bbox_inches="tight")
    # plt.close(fig)
...
def score_and_classify_improved(
    X: np.ndarray,
    X_se: np.ndarray,
    is_ctrl: np.ndarray,
    directions: Sequence[str],
...
def run_multivariate_lme_pipeline_improved(
    config: AnalysisConfig,
    *,
    alpha: float = ALPHA_TARGET,
    use_per_fish_se_in_scoring: bool = USE_PER_FISH_SE_IN_SCORING,
    ...
    scores = score_and_classify_improved(
        X=X,
        X_se=X_se,
        is_ctrl=is_ctrl,
        directions=directions,
    ...
    out = pd.DataFrame(
        {
            ...
            "Is_Learner_Improved": scores.is_learner,
        }
    )
    ...
    if RUN_PLOT_HEATMAP_GRID:
        heatmap_grid_dir = Path(path_pooled_vigor_fig) / "Heatmap_Grids_Improved"
        for cond in config.cond_types:
            save_heatmap_grid(result_obj, config, Path(path_scaled_vigor_fig_cs), heatmap_grid_dir, condition=str(cond))

    return out


if __name__ == "__main__":
    run_multivariate_lme_pipeline_improved(
        analysis_cfg,
        alpha=ALPHA_TARGET,
